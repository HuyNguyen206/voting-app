<div>
    <nav class="hidden lg:flex items-center justify-between text-xs ">
        ...
        <ul class="flex uppercase space-x-10 border-b-4 pb-3">
            ...
            <li  wire:click.prevent="updateStatusFilter('closed',{{request()->routeIs('ideas.show')}})"><a href="" class="@if($status === 'closed') font-semibold text-gray-800 border-blue @endif text-gray-400 transition duration-75 ease-in border-b-4 pb-3 hover:border-blue">Closed ({{ $statusCount['Close'] ?? 0}})</a></li>
        </ul>
    </nav>
</div>
